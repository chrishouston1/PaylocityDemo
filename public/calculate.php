            ...
            <div class="container">
                ...
                if (isset($_GET['employee_id'])) { ?>
                    ...
                    <h1 class="orange-title">
                        <?=$_SESSION['employee']['first_name']?>
                            <?=$_SESSION['employee']['last_name']?>
                    </h1>
                    <?php if($_SESSION['employee']['starts_with_a'] === true) { ?>
                    <p class="error-message-center">(10% discount)</p>
                    <?php } ?>
                ...
                if ($_SESSION['employee']['number_dependents'] > 0) { ?>
                        ...
                        $dependent = 'dependent_' . $i ?>
                            <li>
                                <?=$_SESSION[$dependent]['first_name']?>
                                    <?=$_SESSION[$dependent]['last_name']?>
                                        <?php if($_SESSION[$dependent]['first_name'][0] == 'A' || $_SESSION[$dependent]['first_name'][0] == 'a') { ?>
                    ...
                    ?>
                                <?=$_SESSION['employee']['number_dependents']?> dependent(s) cost: $
                                    <?=$dependents_cost?>/year at $500/dependent.
                            </li>
                        </ul>
                        ...
                        <ul class="dependent-list">
                            ...
                            if($_SESSION['employee']['number_discounts'] > 0) { ?>
                                <li>
                                    <?=$_SESSION['employee']['number_discounts']?> dependent(s) first name starts with A. 10% off becomes $450/year for each dependent with qualifying discount. </li>
                                <?php }
                            } else { ?>
                                <li>No discounts.</li>
                                <?php } ?>
                        ...
                        <ul class="dependent-list">
                            <li>Dependent costs: $
                                <?=$_SESSION['employee']['dependent_cost']?>/year.</li>
                            <li>Total deductions: $
                                <?=$_SESSION['employee']['dependent_cost'] + 1000?>/year.</li>
                            <li>Total deductions per paycheck: $
                                <?= round(($_SESSION['employee']['dependent_cost'] + 1000)/26,2,PHP_ROUND_HALF_UP)?>.</li>
                            <li>Paycheck after deductions: $
                                <?= 2000 - round(($_SESSION['employee']['dependent_cost'] + 1000)/26,2,PHP_ROUND_HALF_UP)?>.</li>
                        </ul>
                        ...
                        <!-- If user goes to page without loading a user, show message -->
